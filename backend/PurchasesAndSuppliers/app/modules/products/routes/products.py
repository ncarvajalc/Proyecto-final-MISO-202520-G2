...
async def obtener_localizacion_producto(
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            inventory_base_url = INVENTORY_SERVICE_URL.rstrip("/")
            product_inventory_url = f"{inventory_base_url}/producto/{sku}"

            product_inventory_response = await client.get(product_inventory_url)
            if product_inventory_response.status_code == 200:
                product_inventory = product_inventory_response.json()
            ...
            if product_inventory_response.status_code == 404:
                return {
                    "sku": sku,
                    "bodega": "",
                    "zona": "",
            ...
            # Si la búsqueda directa falla, se usa el método original recorriendo el inventario completo
