...
try:
    ...
    from app.modules.territories.models.territories_model import (
        Territorio,
        TerritoryType,
    )
except ImportError:
    ...
    logging.warning(
        "Asegúrate de que 'TerritoryType' (Enum) esté definido en 'territories_model.py'"
    )
...
TERRITORIOS_DATA = {
    ...
    "Perú": {
        ...
        "states": {
            ...
            "Arequipa": {"id": str(uuid.uuid4()), "cities": ["Arequipa", "Yanahuara"]},
        },
    },
    "Ecuador": {
        "id": str(uuid.uuid4()),
        "states": {
            "Pichincha": {"id": str(uuid.uuid4()), "cities": ["Quito", "Sangolquí"]},
            "Guayas": {"id": str(uuid.uuid4()), "cities": ["Guayaquil", "Durán"]},
        },
    },
    "México": {
        "id": str(uuid.uuid4()),
...
HOSPITALES_DATA = [
    ...
    {
        "nombre": "Clínica de Las Américas",
        "nit": "890.900.380-2",
        "ciudad": "Medellín",
    ...
    # Perú
    {
        ...
        "nit": "20100063223",
        "ciudad": "Lima",
        "direccion": "Av. Alfredo Salazar 350, San Isidro",
    },
    {
    ...
    # Ecuador
    {
        ...
        "nit": "1790000123001",
        "ciudad": "Quito",
        "direccion": "Av. Mariana de Jesús s/n",
    },
    {
        "nombre": "Clínica Kennedy",
        "nit": "0990000456001",
        "ciudad": "Guayaquil",
        "direccion": "Av. Rodolfo Baquerizo Nazur y C. Crotos (Alborada)",
    },
    # México
    {
        "nombre": "Hospital ABC",
        ...
        "direccion": "Sur 136 # 116, Col. Las Américas (Campus Observatorio)",
    },
    {
        "nombre": "Hospital Ángeles Lomas",
        "nit": "HAL950101-BBB",
        "ciudad": "Ciudad de México",
        "direccion": "Vialidad de la Barranca 240, Huixquilucan",
    },
    {
        "nombre": "Hospital Zambrano Hellion",
        "nit": "HZH000101-CCC",
        "ciudad": "Monterrey",
        "direccion": "Av. Batallón de San Patricio 112, San Pedro Garza García",
    },
    {
        "nombre": "Hospital San Javier",
        "nit": "HSJ-GDL-987",
        "ciudad": "Guadalajara",
        "direccion": "Av. Pablo Casals 640, Prados de Providencia",
    },
]
...
def seed_territorios(db: Session) -> dict:
    ...
    try:
        ...
        if count > 0:
            ...
            ciudades_db = (
                db.query(Territorio).filter(Territorio.type == TerritoryType.CITY).all()
            )
            ...
            for estado_nombre, estado_data in pais_data["states"].items():
                ...
                for ciudad_nombre in estado_data["cities"]:
                    ...
                    if ciudad_nombre in city_map:
                        log.warning(
                            f"Nombre de ciudad duplicado: {ciudad_nombre}. Usando el último ID."
                        )
...
def seed_institutional_clients(db: Session, city_map: dict) -> dict:
    ...
    if not city_map:
        log.error(
            "No se proporcionó mapa de ciudades (city_map). No se pueden sembrar clientes."
        )
    ...
    try:
        ...
        for hosp in HOSPITALES_DATA:
            ...
            territory_id_uuid = (
                uuid.UUID(territory_id_str) if territory_id_str else None
            )
        ...
        if created_nits:
            ...
            new_clients = (
                db.query(InstitutionalClient)
                .filter(InstitutionalClient.identificacion_tributaria.in_(created_nits))
                .all()
            )
...
SALESPEOPLE_DATA = [
    ...
    },  # Ciudad
    {
        ...
        "email": "carla.ruiz@company.com",
        "hire_date": "2023-05-20",
        "status": "Active",
        "territory_name": "Lima",
    },  # Ciudad
    {
        "full_name": "Miguel Soto (Ecuador)",
        "email": "miguel.soto@company.com",
        "hire_date": "2023-02-01",
        "status": "Active",
        "territory_name": "Quito",
    },  # Ciudad
    {
        ...
        "status": "Active",
        "territory_name": "Guadalajara",
    },  # Ciudad (Antes Jalisco)
    {
        "full_name": "Maria Rodriguez (Bogotá)",
...
def seed_salespeople(db: Session, city_map: dict) -> dict:
    ...
    try:
        ...
        for data in SALESPEOPLE_DATA:
            ...
            if not territory_id:
                log.warning(
                    f"No se encontró ID para territorio: {territory_name}. Vendedor '{data['full_name']}' se creará sin territory_id."
                )
        ...
        if created_count > 0:
            ...
            for data in SALESPEOPLE_DATA:
                ...
                if email not in salespeople_map:  # Si no lo encontramos antes
                    new_sp = (
                        db.query(Salespeople).filter(Salespeople.email == email).first()
                    )
                    if new_sp:
...
ROUTES_DATA = [
    ...
    {
        "salesperson_email": "ana.garcia@company.com",
        "institution_nit": "860.010.518-1",
        "day": "2025-10-27",
        "done": 0,
    },  # Visita a Santa Fe
    {
        "salesperson_email": "ana.garcia@company.com",
        "institution_nit": "860.010.518-2",
        "day": "2025-10-28",
        "done": 0,
    },  # Visita a San Ignacio
    # Rutas para Luis Pérez (Medellín)
    {
        "salesperson_email": "luis.perez@company.com",
        "institution_nit": "890.900.380-1",
        "day": "2025-10-27",
        "done": 0,
    },  # Visita a Pablo Tobón
    # Rutas para Carla Ruiz (Lima)
    {
        "salesperson_email": "carla.ruiz@company.com",
        "institution_nit": "20100063223",
        "day": "2025-10-29",
        "done": 0,
    },  # Visita a Clínica Anglo Americana
    # Rutas para Javier Torres (Guadalajara)
    {
        "salesperson_email": "javier.torres@company.com",
        "institution_nit": "HSJ-GDL-987",
        "day": "2025-10-27",
        "done": 0,
    },  # Visita a Hospital San Javier
]
...
def seed_routes(db: Session, salespeople_map: dict, institution_map: dict):
    ...
    if not institution_map:
        log.error(
            "No se proporcionó mapa de instituciones. No se pueden sembrar rutas."
        )
        return
    ...
    try:
        ...
        for route_data in ROUTES_DATA:
            ...
            if not salesperson_id or not institution_id:
                log.warning(
                    f"No se encontró ID para vendedor ({email_vendedor}) o institución ({nit_institucion}). Saltando ruta."
                )
                continue
