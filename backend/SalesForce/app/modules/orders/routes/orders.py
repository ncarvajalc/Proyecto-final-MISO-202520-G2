...
from app.modules.orders.services import create_order_service, get_top_purchased_products
...
def read_most_purchased_products(
    page: int = 1,
    limit: int = 20,
    db: Session = Depends(get_db)
):
    return get_top_purchased_products(db=db, page=page, limit=limit)
