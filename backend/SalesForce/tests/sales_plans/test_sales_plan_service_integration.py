...
def test_list_sales_plans_returns_paginated_response(db_session, fake: Faker):
    ...
    baseline_total = service.list_sales_plans(db_session, page=1, limit=1).total
    ...
    assert all(plan.vendedor_nombre for plan in first_page.data)

    stored = db_session.query(Salespeople).filter_by(id=salesperson.id).first()
    assert stored is not None
    assert len(stored.sales_plans) >= 3
