...
def create(db: Session, client: InstitutionalClientCreate) -> InstitutionalClient:
...
async def list_clients_territories(
    ...
    result = (
        ...
        if territories
        else list_institutional_clients_paginated(
            db, skip=skip, limit=limit, search=search
        )
    )
    ...
    if not result or "total" not in result or "items" not in result:
        print(
            "Error: list_institutional_clients_paginated devolvió un formato inesperado."
        )
    ...
    clients = [
        InstitutionalContactClient.model_validate(item) for item in result["items"]
    ]

    territory_ids = [client.territory_id for client in clients if client.territory_id]
    ...
    if territory_ids:
        ...
        if lineage_map:
            ...
            print(
                "No se pudo obtener el linaje de los territorios (lineage_map está vacío o es None)."
            )
    else:
...
async def obtener_territorios_by_ids(
    client: httpx.AsyncClient, territory_ids: List[str]
    ...
    try:
        response = await client.post(
            f"{TERRITORY_SERVICE_URL}/territorios/lineages-by-ids", json=payload
        )
        ...
        print(
            f"Error HTTP de la API de linajes: {e.response.status_code} - {e.response.text}"
        )
        return None
